  ...
  container.innerHTML = leads.map(lead => {
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden" id="offer-lead-${lead.id}">
      <div class="p-4 sm:p-5 flex flex-col lg:flex-row gap-4">
        ...
        <div class="lg:w-80 shrink-0 space-y-2.5">
          <div class="flex items-start justify-between gap-2">
            <div class="flex items-start gap-2.5">
              ...
              <div>
                <h4 class="font-black text-sm text-slate-900 leading-snug" title="${esc(lead.company_name)}">${esc(lead.company_name)}</h4>
                <div class="flex flex-wrap gap-1 mt-1">
                  ...
                  ${lead.website
                    ? `<span class="text-[10px] font-bold px-1.5 py-0.5 rounded-md bg-emerald-50 text-emerald-700 border border-emerald-100" title="${esc(lead.website)}">🌐 есть сайт</span>`
          ...
          ${lead.phone ? `
            <div class="flex items-center gap-2">
              <a href="tel:${esc(lead.phone.replace(/[^\d+]/g, ''))}" class="font-black text-base text-brand-deep hover:text-brand-blue transition-colors">📞 ${esc(lead.phone)}</a>
              <button onclick="copyOffersText('${esc(lead.phone)}', this)" title="Скопировать номер"
                class="p-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-500 transition-colors">
                <i data-lucide="copy" class="w-3.5 h-3.5"></i>
              </button>
            </div>
          ` : `<p class="text-xs text-slate-400 italic">Телефон не указан</p>`}

          ${lead.address ? `<p class="text-[11px] text-slate-500 leading-snug">${esc(lead.address)}</p>` : ''}
          ${lead.extra_info ? `<p class="text-[10px] text-slate-400 leading-snug line-clamp-2" title="${esc(lead.extra_info)}">${esc(lead.extra_info)}</p>` : ''}
        ...
        <div class="flex-1 min-w-0">
          ${hasOffer ? `
            <div class="rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-50/80 via-white to-blue-50/50 p-4 space-y-3 h-full">
              ...
              <div class="space-y-2.5 text-xs leading-relaxed max-h-72 overflow-y-auto pr-1 offers-offer-scroll">
                ${lead.offer_hook ? `
                  <div>
                    <p class="text-[10px] font-black uppercase tracking-wider text-rose-500 mb-0.5">🪝 Крючок · первые секунды</p>
                    <p class="text-slate-800 font-medium">${esc(lead.offer_hook)}</p>
                  </div>
                ` : ''}
                ${lead.offer_text ? `
                  <div>
                    <p class="text-[10px] font-black uppercase tracking-wider text-violet-600 mb-0.5">💼 Оффер</p>
                    <p class="text-slate-800 font-medium">${esc(lead.offer_text)}</p>
                  </div>
                ` : ''}
                ${lead.offer_objections ? `
                  <div>
                    <p class="text-[10px] font-black uppercase tracking-wider text-amber-600 mb-0.5">🛡 Если возражают</p>
                    <p class="text-slate-700">${esc(lead.offer_objections)}</p>
                  </div>
                ` : ''}
                ${lead.offer_closing ? `
                  <div>
                    <p class="text-[10px] font-black uppercase tracking-wider text-emerald-600 mb-0.5">✅ Закрытие</p>
                    <p class="text-slate-800 font-semibold">${esc(lead.offer_closing)}</p>
                  </div>
                ` : ''}
              </div>
      ...
      <!-- Нижняя панель: удаление -->
      <div class="bg-slate-50 border-t border-slate-100 px-4 py-2 flex items-center justify-between">
        <span class="text-[10px] text-slate-400 font-mono">#${lead.id}${lead.source_file ? ` · ${esc(lead.source_file)}` : ''}</span>
        <button onclick="deleteOfferLead(${lead.id})" class="text-[11px] font-bold text-slate-400 hover:text-rose-600 transition-colors flex items-center gap-1">
